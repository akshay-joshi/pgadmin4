...
describe('BgProcessManager', ()=>{
  ...
  it('openProcessesPanel', ()=>{
    const panel = {};
    jest.spyOn(pgBrowser.docker.side_workspace, 'openTab').mockReturnValue(panel);

    /* panel open */
    jest.spyOn(pgBrowser.docker.side_workspace, 'find').mockReturnValue(panel);
    jest.spyOn(pgBrowser.docker.side_workspace, 'focus');
    obj.openProcessesPanel();
    expect(pgBrowser.docker.side_workspace.focus).toHaveBeenCalled();
    expect(pgBrowser.docker.side_workspace.openTab).not.toHaveBeenCalled();

    /* panel closed */
    jest.spyOn(pgBrowser.docker.side_workspace, 'find').mockReturnValue(null);
    obj.openProcessesPanel();
    expect(pgBrowser.docker.side_workspace.openTab).toHaveBeenCalled();
  });
});
